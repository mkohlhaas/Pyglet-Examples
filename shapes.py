...
class ShapesDemo(Window):
    def __init__(self, width, height):
        ...
        self.circle = Circle(
            ...
            240,
            75,
            color=(255, 225, 255, 127),
            batch=self.batch,
        )

        # Rectangle with center as anchor
        self.square = BorderedRectangle(
            360,
            240,
            100,
            100,
        ...
        self.rectangle = Rectangle(
            100, 190, 500, 100, color=(255, 22, 20, 64), batch=self.batch
        )

        self.line = Line(
            0, 0, 0, 480, thickness=4, color=(200, 20, 20), batch=self.batch
        )

        self.triangle = Triangle(
            10, 10, 190, 10, 100, 150, color=(55, 255, 255, 175), batch=self.batch
        )

        septagon_step = math.pi * 2 / 7
        self.fading_septagon = Polygon(
            *[
                [
                    50 + 40 * math.sin(i * septagon_step),
                    200 + 40 * math.cos(i * septagon_step),
                ]
                for i in range(7)
            ],
            batch=self.batch,
        ...
        self.star = Star(600, 375, 50, 30, 5, color=(255, 255, 0), batch=self.batch)

        self.ellipse = Ellipse(
            650, 150, a=50, b=30, color=(55, 255, 55), batch=self.batch
        )

        self.sector = Sector(
            125, 400, 60, angle=0.45 * 360, color=(55, 255, 55), batch=self.batch
        )

        self.polygon = Polygon(
            [400, 100], [500, 10], [600, 100], [550, 175], [450, 150], batch=self.batch
        )

        self.box = Box(
            60, 40, 200, 100, thickness=2, color=(244, 55, 55), batch=self.batch
        )

        coordinates = [[450, 400], [475, 450], [525, 450], [550, 400]]
        self.multiLine = MultiLine(*coordinates, closed=True, batch=self.batch)
...
if __name__ == "__main__":
    ...
    clock.schedule_interval(demo.update, 1 / 30)
    app.run()
