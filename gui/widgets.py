...
from pyglet.gui import Frame, PushButton, Slider, TextEntry, ToggleButton
from pyglet.resource import image
from pyglet.text import Label
from pyglet.window import Window

window = Window(540, 500, caption="Widget Example")
batch = Batch()
...
unpressed = image("button_unpressed.png")
...
def slider_handler(_widget, value):
    slider_label.text = f"Slider Value: {round(value, 1)}"


def toggle_button_handler(_widget, value):
    toggle_label.text = f"Toggle Button: {value}"


def push_button_handler(_widget):
    push_label.text = "Push Button: True"


def release_button_handler(_widget):
    push_label.text = "Push Button: False"


def text_entry_handler(_widget, text):
...
slider = Slider(100, 200, bar, knob, edge=5, batch=batch)
